...
async function handleCronNotification(request: Request) {
  try {
    ...
    // Check if reminders are stopped/disabled (unless doing a manual test)
    ...
    // Check if current PKT hour matches configured reminder time (unless doing a manual test)
    if (!isTest) {
      const targetTimeStr = settings.reminder_time || '20:00'; // e.g. "20:00"
